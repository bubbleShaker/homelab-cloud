...
- **焼き込み手順（Raspberry Pi Imager）**:
  ...
  2. **⚙ 詳細設定（歯車）** で以下を事前投入 → これがヘッドレス化の肝:
     - hostname: `hlc-node1`
     - **SSH を有効化 + 公開鍵認証**（`sshd_config` の `PasswordAuthentication no` でパスワード認証を無効化。アカウント自体のパスワードは残してよい）
...
- [ ] `ssh hlc-node1.local` に**公開鍵で**ログインできる（`sshd_config` の `PasswordAuthentication no` を確認）
